...
class Reaction:
    ...
    @classmethod
    async def get_by_gcid(cls, emoji: str, gc_sender: str, gc_msgid: str, gc_chat: str, gc_receiver: str) -> Optional['Reaction']:
        q = (f"SELECT {cls.columns} FROM reaction "
    ...
    async def insert(self) -> None:
        q = ("INSERT INTO reaction (mxid, mx_room, emoji, gc_sender, gc_msgid, gc_chat,"
             "                      gc_receiver, timestamp) "
             "VALUES ($1, $2, $3, $4, $5, $6, $7, $8)")
        await self.db.execute(q, self.mxid, self.mx_room, self.emoji, self.gc_sender,
                              self.gc_msgid, self.gc_chat, self.gc_receiver, self.timestamp)

    async def delete(self) -> None:
        q = ("DELETE FROM reaction WHERE emoji=$1 AND gc_sender=$2 AND gc_msgid=$3"
             "                       AND gc_chat=$4 AND gc_receiver=$5")
        await self.db.execute(q, self.emoji, self.gc_sender, self.gc_msgid, self.gc_chat,
                              self.gc_receiver)
